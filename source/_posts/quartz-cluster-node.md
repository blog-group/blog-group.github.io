...
---
在上一章{% post_link quartz-signle-node Quartz分布式单节点持久化任务 %}中我们已经完成了任务的持久化，当我们创建一个任务时任务会被``quartz``定时任务框架自动持久化到数据库，我们采用的是``SpringBoot``项目托管的``dataSource``来完成的数据源提供，当然也可以使用``quartz``内部配置数据源方式，我们的标题既然是提到了定时任务的分布式多节点，那么怎么才算是多节点呢？当有节点故障或者手动停止运行后是否可以自动漂移任务到可用的分布式节点呢？
<!--more-->
# 本章目标
